...
export function CardStatisticsTable({ cards, decks }: { cards: CardStatistics[]; decks: Statistics["decks"] }) {
  ...
  return <section className="statistics-section" aria-labelledby="card-history-title">
    <div className="statistics-section-heading"><div>
      <h2 id="card-history-title">Card history</h2><p>All-time reviews and the current schedule for each puzzle.</p>
    </div></div>
    ...
    {rows.length === 0 ? <p className="statistics-empty" role="status">No cards match this filter.</p> : <>
      ...
      <div className="statistics-pagination">
        ...
        <div><Button disabled={currentPage === 0} onClick={() => setPage(currentPage - 1)} aria-label="Previous page of cards">Previous</Button><Button disabled={currentPage === lastPage} onClick={() => setPage(currentPage + 1)} aria-label="Next page of cards">Next</Button></div>
      </div>
    </>}
